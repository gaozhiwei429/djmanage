...
<div class="layui-card">
    <div class="layui-header notselect">
        ...
        <div class="pull-right margin-right-15 nowrap">
            <?php
            if(isset($menuUrl) && !empty($menuUrl) && in_array(trim(Url::to(['admin/gift/edit']),"/"), $menuUrl)) {
                ?>
                <button data-open='<?=Url::to(['admin/gift/edit']);?>' class='layui-btn layui-btn-sm layui-btn-primary'>添加</button>
            <?php
            }
            ?>
        </div>
...
<script type="application/javascript">
    layui.use(['form', 'table', 'laypage', 'layer'], function(){
        ...
        $.ajax({
            type: "post",
            url:"<?= Url::to(['common/gift-icon/get-list']); ?>",
...
<script type="text/html" id="Jimg">
    {{#  if(d.pic_url!=""){ }}
    <img src="{{d.pic_url}}" style="height:30px;width:30px;border-radius:50%;line-height:50px!important;">
    {{# }else{ }}
    暂无
    {{#  } }}
</script>
<script type="text/html" id="barDemo">
    ...
    if(isset($menuUrl) && !empty($menuUrl) && in_array(trim(Url::to(['admin/gift/edit']),"/"), $menuUrl)) {
        ?>
        <a class="layui-btn layui-btn-radius layui-btn-sm layui-btn-danger" lay-event="edit" data-open="<?=Url::to(['admin/gift/edit']);?>?id={{d.id}}">编辑</a>
    <?php
    }
    ?>
</script>