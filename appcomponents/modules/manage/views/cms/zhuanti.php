...
<div class="layui-card">
    <div class="layui-header notselect">
        ...
        <div class="pull-right margin-right-15 nowrap">
            <?php
//            if(isset($menuUrl) && !empty($menuUrl) && in_array(trim(Url::to(['manage/cms/zhuanti-edit']),"/"), $menuUrl)) {
                ?>
                <button data-open="<?=Url::to(['manage/cms/zhuanti-edit']);?>?utilization_id=<?=isset($utilization_id) ? $utilization_id : 0;?>" class='layui-btn layui-btn-sm layui-btn-primary'>添加专题</button>
            <?php
//            }
            ?>
            <?php
            if(isset($menuUrl) && !empty($menuUrl) && in_array(trim(Url::to(['manage/cms/zhuanti-del']),"/"), $menuUrl)) {
                ?>
                <button data-update data-field='delete' data-action='<?=Url::to(['manage/cms/zhuanti-del']);?>'  class='layui-btn layui-btn-sm layui-btn-primary'>删除</button>
            <?php
            }
            ?>
        </div>
...
layui.use(['form', 'table', 'laypage', 'layer'], function(){
    ...
    params.size = size;
    params.count = 0;
    params.utilization_id = "<?=$utilization_id ? $utilization_id : 0;?>";
    form.on('submit(commit)', function(data){
        ...
        selfUrl = changeURLArg(selfUrl,"utilization_id","<?=$utilization_id ? $utilization_id : 0;?>");
        location.href = selfUrl;
    });
    $.ajax({
        type: "post",
        url:"<?= Url::to(['common/zhuanti/get-list']); ?>",
...
//                        ,cellMinHeight: 500
    ...
    table.on('edit(text)', function(obj){ //注：edit是固定事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        $.ajax({
            type: "post",
            url: "<?= Url::to(['common/zhuanti/set-sort']); ?>",
            contentType: "application/json;charset=utf-8",
            data: JSON.stringify(params),
            dataType: "json",
            beforeSend: function (XMLHttpRequest) {
    ...
    form.on('switch(status)', function(obj){
        ...
        $.ajax({
            type: "post",
            url: "<?= Url::to(['common/zhuanti/set-status']); ?>",
            contentType: "application/json;charset=utf-8",
            data: JSON.stringify(params),
            dataType: "json",
            beforeSend: function (XMLHttpRequest) {
...
<script type="text/html" id="barDemo">
    <a class="layui-btn layui-btn-normal layui-btn-xs" data-open="<?=Url::to(['manage/cms/zhuanti-edit']);?>?id={{d.id}}&utilization_id=<?=isset($utilization_id) ? $utilization_id : 0;?>">
        修改
    </a>
