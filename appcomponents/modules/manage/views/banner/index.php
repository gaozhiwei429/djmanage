    ...
    <div class="layui-header notselect">
        ...
        <div class="pull-right margin-right-15 nowrap">
            ...
            if(isset($menuUrl) && !empty($menuUrl) && in_array(trim(Url::to(['manage/banner/add']),"/"), $menuUrl)) {
                ?>
                <button data-open='<?=Url::to(['manage/banner/edit']);?>' class='layui-btn layui-btn-sm layui-btn-primary'>添加</button>
            <?php
            }
            ?>
            <?php
            if(isset($menuUrl) && !empty($menuUrl) && in_array(trim(Url::to(['manage/banner/del']),"/"), $menuUrl)) {
                ?>
                <button data-update data-field='delete' data-action='<?=Url::to(['manage/banner/del']);?>'  class='layui-btn layui-btn-sm layui-btn-primary'>删除</button>
            <?php
            }
            ?>
        </div>
...
<script type="application/javascript">
    layui.use(['form', 'table', 'laypage', 'layer'], function(){
        ...
        $.ajax({
            type: "post",
            url:"<?= Url::to(['common/banner/get-banners']); ?>",
...
<script type="text/html" id="barDemo">
    <?php
    if(isset($menuUrl) && !empty($menuUrl) && in_array(trim(Url::to(['manage/banner/info']),"/"), $menuUrl)) {
        ?>
        <a class="layui-btn layui-btn-radius layui-btn-sm layui-btn-mini" lay-event="edit" data-open="<?=Url::to(['manage/banner/info']);?>?id={{d.id}}">查看</a>
    <?php
    }
    ?>
    <?php
