...
layui.use(['form', 'table', 'laypage', 'layer'], function(){
    ...
    var table = layui.table
        ...
        ,$ = layui.jquery
        ,layer = layui.layer;
    $.cookie('sectionIds', "<?=isset($sectionIds) ? $sectionIds : ""?>");
    ...
    $.ajax({
        type: "post",
        url:"<?= Url::to(['common/sections/get-list']); ?>",
            ...
            if(result.code == 0) {
                ...
                table.render({
                    ...
                    ,done: function(res, curr, count){
                        var checkboxArr = $.cookie('sectionIds').split(',');
                        ...
                        laypage.render({
                            ...
                            ,jump : function(obj, first){
                                if($.cookie('sectionIds') !="" && $.cookie('sectionIds') != undefined) {
                                    selfUrl = changeURLArg(selfUrl,'sectionIds',$.cookie('sectionIds'));
                                }
    ...
    table.on('checkbox(text)', function(obj){
        if($.cookie('sectionIds') !="" && $.cookie('sectionIds') != undefined) {
            emp = $.cookie('sectionIds').split(',');
        }
        if(obj.checked==true) {
            emp.push(obj.data.id);
            $.cookie('sectionIds', emp.join(','));
        } else {
            emp.pop(obj.data.id);
            $.cookie('sectionIds', emp.join(','));
        }
        $("#JtableIds").val($.cookie('sectionIds'));
