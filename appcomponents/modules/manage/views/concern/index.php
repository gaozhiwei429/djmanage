...
<div class="layui-card">
    ...
    <div class="layui-btn-group">
        ...
        <button class="layui-btn">取消分组</button>
        <button class="layui-btn">移动分组</button>
        <button class="layui-btn">取消关注</button>
    </div>
</div>
...
//        selfUrl+=paramsStr;
    ...
    $.ajax({
        type: "post",
        url:"<?= Url::to(['common/concern/get-list']); ?>",
            ...
            if(result.code == 0) {
                table.render({
                    ...
                    ,cols: [[
                        ...
                        ,{field:'numberstr', title: '打印机序列号'}
                        ,{field:'model_name', title: '型号'}
                        ,{field:'print_name', title: '打印机重命名'}
                        ,{field:'bd_diqu_name', title: '地区'}
                        ,{field:'mac', title: 'mac地址'}
                        ,{field:'machine_date', title: '生产日期'}
...
<script type="text/html" id="JconcernStatus">
    ...
    <input type="checkbox" name="concern_status" class="layui-btn layui-btn-primary" data-status="{{d.concern_status}}" value="{{d.mac}}" title="取消" lay-filter="concern_status" checked>
    <!--    <button class="layui-btn layui-btn-primary" data-id="{{d.id}}"  name="concern_status" lay-skin="concern_status" >已关注</button>-->
    {{# }else{ }}
    <input type="checkbox" name="concern_status" class="layui-btn layui-btn-danger" data-status="{{d.concern_status}}" value="{{d.mac}}" title="关注" lay-filter="concern_status">
