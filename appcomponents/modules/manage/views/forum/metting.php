            ...
            if(result.code == 0) {
                table.render({
                    ...
                    ,cols: [[
                        ...
                        ,{field:'type', title: '所属类型', minWidth: 80}
...
<script type="text/html" id="Jstatus">
    {{#  if(d.status ==0){ }}
<input type="checkbox" name="status" lay-skin="switch" lay-text="下线|上线" lay-filter="status" data-id="{{d.id}}" data-status="{{d.status}}" value="{{d.id}}"/>
    {{# }else if(d.status ==1) { }}
