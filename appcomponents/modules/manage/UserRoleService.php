...
use appcomponents\modules\manage\models\UserRoleModel;
...
class UserRoleService extends BaseService {
    ...
    public function getDatas($params, $orderBy = [], $offset=0, $limit=10, $field=['*']) {
        $adminUserRegionModel = new UserRoleModel();
     ...
     * @return array
     */
    public function editUserRole($user_id, $roleIds) {
        $userRoleListRet = $this->getUserRoleIds($user_id);
        $roleIdArr = [];
        ...
        $add = true;
        if(!empty($roleIdArr)) {
            $updateAllParams['user_id'] =  $user_id;
            $updateData['status'] = 0;
