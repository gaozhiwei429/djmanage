...
<div class="layui-card">
    <div class="layui-header notselect">
        ...
        <div class="pull-right margin-right-15 nowrap">
            <?php
            if(isset($menuUrl) && !empty($menuUrl) && in_array(trim(Url::to(['admin/manage-user/add']),"/"), $menuUrl)) {
                ?>
                <button data-modal='<?=Url::to(['admin/manage-user/edit']);?>' class='layui-btn layui-btn-sm layui-btn-primary'>添加账号</button>
            <?php
            }
            ?>
            <?php
            if(isset($menuUrl) && !empty($menuUrl) && in_array(trim(Url::to(['admin/manage-user/del']),"/"), $menuUrl)) {
                ?>
                <button data-update data-field='delete' data-modal='<?=Url::to(['admin/manage-user/del']);?>'  class='layui-btn layui-btn-sm layui-btn-primary'>删除账号</button>
            <?php
            }
            ?>
        </div>
...
<script type="text/html" id="barDemo">
    ...
    <?php
    if(isset($menuUrl) && !empty($menuUrl) && in_array(trim(Url::to(['admin/manage-user/info']),"/"), $menuUrl)) {
        ?>
<!--        <a class="layui-btn layui-btn-radius layui-btn-sm layui-btn-mini" data-modal="--><?//=Url::to(['/manage-user/info']);?><!--?id={{d.id}}">查看</a>-->
    <?php
    }
    ?>
    <?php
    if(isset($menuUrl) && !empty($menuUrl) && in_array(trim(Url::to(['admin/manage-user/edit']),"/"), $menuUrl)) {
        ?>
        <a class="layui-btn layui-btn-radius layui-btn-sm layui-btn-mini" data-modal="<?=Url::to(['/manage-user/edit']);?>?id={{d.id}}">编辑</a>
    <?php
    }
    ?>
    <?php
    if(isset($menuUrl) && !empty($menuUrl) && in_array(trim(Url::to(['admin/manage-user/pass']),"/"), $menuUrl)) {
        ?>
        <a class="layui-btn layui-btn-radius layui-btn-sm layui-btn-danger" data-title="修改密码" data-modal="<?=Url::to(['/manage-user/pass']);?>?id={{d.id}}">重置密码</a>
    <?php
    }
    ?>
</script>