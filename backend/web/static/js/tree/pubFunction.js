...
fun.openWindowOrgUser = function(page,strType,strIsMyParty,strSingle){
	...
	top.openWindowNew({
	    ...
	    area: [nWidth, '550px'],
	    closeBtn :0,//不显示关闭按钮
	    content: "window/selectOrgEmployeeWindowAction.action?strType="+strType+"&strIsMyParty="+strIsMyParty+"&strSingle="+strSingle+strText
	},page,strBack);
}
...
//弹窗方法
function warningMsg(message,callback){
	if(parseInt(arguments.length) == 2){
		layer.msg(message, {
			  icon:0,
			  time: msgtime
...
//全局自动关闭通知菜单
$('body :not(:has(.right-sidebar-toggle),.right-sidebar-toggle,#right-sidebar,.sidebar-container,.sidebar-container ul,.sidebar-container ul li,.tab-content,.tab-content ul,.tab-content ul li)').click(function(){
	try{
...
function initFun(){
	//重新赋值用户帐号别名
	if ($(".userCodeTitle").length > 0 && top.userCodeTitle.length > 0){
...
function downLoadFile(fileGuid){
	downloadFileUrl = "base/fileInfoAction!downLoadFile.action";
	//post提交方式
